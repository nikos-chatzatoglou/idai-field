...
global.setConfigDefaults = (config) => {
    ...
    setFileSync(config);
    if (config.isAutoUpdateActive === undefined)
        config.isAutoUpdateActive = true;
    if (config.highlightCustomElements === undefined)
        config.highlightCustomElements = true;
    setLanguages(config);
...
const setFileSync = (config) => {
    Object.values(config.syncTargets).map((target) => {
        if (
            typeof target.activeFileSync !== "undefined" &&
            target.activeFileSync.every((i) => typeof i === "string")
        ) {
            let updatedConfig = [];
...
const setLanguages = (config) => {
    ...
    config.languages = config.languages.concat(
        mainLanguages.filter((language) => !config.languages.includes(language))
    );
};

const getLocale = () => {
    return electron.app.getLocale().replace("_", "-").split("-")[0];
};

global.getLocale = () =>
    global.config.languages.find((language) =>
...
if (["production", "development"].includes(global.mode)) {
    ...
    global.appDataPath =
        electron.app.getPath("appData") + "/" + electron.app.getName();
...
const createMenu = () => {
    const menu = electron.Menu.buildFromTemplate(
        require("./menu.js")(mainWindow, menuContext, global.config)
    );
...
electron.ipcMain.on("reload", (event, route) => {
    if (global.mode === "production") {
        mainWindow.loadURL(
            url.format({
                pathname: require("path").join(
                    __dirname,
                    "/../dist/" + global.getLocale() + "/index.html"
                ),
                protocol: "file:",
                slahes: true,
                hash: route,
