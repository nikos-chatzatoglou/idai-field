...
export class MultiLanguageTextFieldComponent implements OnChanges {
    ...
    @Output() onFieldDataChanged: EventEmitter<
        I18N.String | string | undefined
    > = new EventEmitter<I18N.String | string | undefined>();

    @ViewChild('multiLanguageTextField')
    multiLanguageTextFieldElement: ElementRef;
    ...
    public isFilledIn = (languageCode: string) =>
        this.multiLanguageText?.[languageCode] !== undefined;

    public hasTabs = () =>
        this.languages.length > 1 ||
        (this.languages.length === 1 &&
            this.languages[0].code !== I18N.UNSPECIFIED_LANGUAGE);

    ngOnChanges(changes: SimpleChanges) {
        if (changes['languages']) {
            this.tabLanguages =
                this.languages.length > 5
                    ? this.languages.slice(0, 4)
                    : this.languages;
            this.additionalLanguages =
                this.languages.length > 5 ? this.languages.slice(4) : [];
            if (this.additionalLanguages.length > 0)
                this.shownAdditionalLanguage = this.additionalLanguages[0];
        }
    ...
    public onAdditionalLanguagesDropdownMenuToggled(opened: boolean) {
        ...
        if (
            this.additionalLanguagesDropdownOpened &&
            this.shownAdditionalLanguage
        ) {
    ...
    public selectLanguage(language: Language) {
        if (this.additionalLanguages.includes(language))
            this.shownAdditionalLanguage = language;
    ...
    public getSelectedLanguageLabel(): string {
        ...
        return language ? language.label : this.selectedLanguage;
    }

    public onMouseDown(event: any) {
        if (
            (this.tabsElement &&
                event.target === this.tabsElement.nativeElement) ||
            !ComponentHelpers.isInside(
                event.target,
                (target) =>
                    target === this.multiLanguageTextFieldElement.nativeElement
            )
        ) {
            this.focused = false;
        }
        ...
        setTimeout(() => {
            if (
                !ComponentHelpers.isInside(
                    ...
                    (target) =>
                        target ===
                        this.multiLanguageTextFieldElement.nativeElement
                )
            ) {
                this.focused = false;
            }
    ...
    private updateMultiLanguageText(value: string) {
        if (
            (this.multiLanguageText && isString(this.multiLanguageText)) ||
            this.hasNoConfiguredLanguages()
        ) {
            this.multiLanguageText = value;
        } else if (value === '') {
            if (this.multiLanguageText) {
                delete this.multiLanguageText[this.selectedLanguage];
                if (isEmpty(this.multiLanguageText))
                    this.multiLanguageText = undefined;
            }
        } else {
